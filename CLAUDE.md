...
## ⭐ 최상위 운영 원칙 — 마태복음 10:16

> **상세 문서: `docs/CORE_VISION.md`** (반드시 정기적으로 재독)

> **"뱀처럼 지혜롭고 비둘기처럼 순결하라"** — 예수님의 전도 전략 (마10:16)
...
| 축 | 의미 | 위반 시 |
|---|---|---|
| **🐍 뱀처럼 지혜롭게** | 시장·문화·인간 본성 통찰 / 효과적 실행 | 순진한 이상론 (영향력 ↓) |
| **🕊️ 비둘기처럼 순결하게** | 동기 순수·조작 ❌·강요 ❌ | 마케팅 회사로 변질 |

**둘의 균형이 그리스도의 환대(hospitality)를 만든다.**

### 세 기둥 (Three Pillars)

| 기둥 | 의미 | 핵심 |
|---|---|---|
| **① 문화 상품 제작** | 콘텐츠 자체가 자랑할 상품 | 검증·편집·큐레이션 |
| **② 글로벌 커뮤니티** | 한국 입구 → 모두에게 열림 | **민족주의 ❌, 보편 환대 ✅** |
| **③ 그리스도의 빛** | 강요 없이 자연스럽게 빛남 | 1·2층 깊어지면 자동 빛남 |

### 작업 전 5가지 질문 (의사결정 프레임)

```
[지혜 질문]
1. 시장·문화·인간 본성을 정확히 이해한 결정인가?
2. 효과적으로 목적을 달성하는가? (자원 낭비 ❌)

[순결 질문]
3. 사람을 수단으로 쓰지 않는가? (조작 ❌)
4. 사용자에게 정직하고 투명한가?

[통합 질문]
5. 그리스도인의 환대를 자연스럽게 보여주는가?
...
- ❌ **민족주의** → 김구·안중근 인용 도배 / "한국 최고" 메시지
- ✅ **보편적 환대** → 한국 문화는 매력적 입구, 모두에게 열림
- ❌ **종교 강요** → 직접 전도·성경구절 도배
- ✅ **자연스러운 빛** → 검증·섬김·환대로 그리스도 보여주기

---

## 🏙️ 다도시 운영 원칙 — 시애틀 기준 + 도시 고유성

> **"기본 원칙은 시애틀을 표준으로 삼되, 각 도시의 고유한 특징은 반드시 살려야 한다."**  
> — 폴 김 목사 (2026-05-08)
...
```
┌─────────────────────────┬──────────────────────────┐
│  ① 시애틀 = 마스터 템플릿   │  ② 각 도시 = 고유성 보존    │
│  (구조·UI·기능 통일)      │  (데이터·시스템·문화 차별화)│
└─────────────────────────┴──────────────────────────┘
```
...
| 항목 | 표준 |
|---|---|
| 탭 구조 | 9개 탭 (홈·정착·교회·맛집·관광·도움·취업·교육·생활비) |
| Quick Menu | 16개 아이콘 4×4 그리드 |
| 더보기 시트 | 10개 빠른 접근 |
| ScreenHeader 디자인 | 동일 (도시명만 동적 치환) |
| Korean American 비전 섹션 | 모든 도시 공통 |
| 검증 표시 체계 | ✅ 검증됨 / 🔗 웹사이트 / Yelp 별점 |
| 카테고리 필터 (맛집) | BBQ/치킨/국물/카페/분식 6개 |

### ② 도시별 고유성 (반드시 보존할 것)

| 영역 | 고유성 예시 |
|---|---|
| **TOP5 맛집** | 시애틀: 이가네·쏘문난집 / 달라스: 장통닭·Korea House / SF: BCD·Sun Nong Dan |
| **TOP5 정착** | 각 도시 한인회·총영사관·주요 H-Mart |
| **TOP5 탐방** | 시애틀: 레이니어·Pike Place / 달라스: AT&T Stadium / NYC: 자유의여신상 |
| **거주지 추천** | 시애틀: 린우드·벨뷰 / 달라스: 캐롤튼·플라노 / LA: 코리아타운·세리토스 |
| **운전면허** | WA DOL (한국어 필기) / TX DPS (90일) / CA DMV (영어만) / Ontario MTO (G 시스템) |
| **세금 시스템** | WA·TX 소득세 ❌ / CA 최대 13.3% / Canada는 OHIP/MSP 무료 의료 |
| **이민 시스템** | US: USCIS·SSN·ITIN / Canada: IRCC·SIN |
| **건강보험** | WA Apple Health / Covered California / NY State of Health / OHIP·MSP |
| **태그라인** | 시애틀: "도시를 알고, 사람을 찾다" / 달라스: "텍사스에서 뿌리내리다" |

### 새 도시 추가 시 체크리스트

```
□ 시애틀 템플릿 그대로 복사 (구조·UI 동일)
□ TOP5_RESTAURANTS_[CITY]: 그 도시 한식당 5선 (Yelp 별점 포함)
□ TOP5_SETTLE_[CITY]: 한인회·H-Mart·DMV·의료·총영사관
□ TOP5_EXPLORE_[CITY]: 그 도시 명소 5선 + 한인타운 필수
□ areasKo/En: 그 도시 한인 거주지 5곳 (렌트·학군·교통)
□ adminKo/En: 그 도시 SSN/SIN·면허·의료·이민 (지역 특화)
□ financeKo/En: 그 도시 세금 특이사항 (소득세 유무·비율)
□ CitySlug 타입에 추가 + CITY_CONFIGS 등록
□ 모든 Top5Banner 분기에 연결
```

### 도시 고유성 위반 금지 사례

❌ **하지 말 것:**
- 모든 도시에 시애틀 식당 표시 (Park's BBQ in LA로 표시 X, LA는 LA 식당으로)
- 모든 도시에 "WA Apple Health" 표시 (CA는 Covered California)
- 모든 도시에 같은 운전면허 절차 (Texas는 90일·WA는 30일·MA는 60일·CA는 10일)
- 모든 도시에 "주 소득세 없음" 표시 (CA·NY는 있음, WA·TX는 없음)

✅ **반드시 할 것:**
- 도시 결정한 정보는 `citySlug` 분기로 처리
- 시애틀 데이터는 default 케이스로 fallback
- 모르는 도시 정보는 추측 금지 → "정보 확인 중" 표시

### 현재 구현 상태 (2026-05-08)

| 도시 | UI 표준화 | TOP5 맛집 | TOP5 정착 | TOP5 탐방 | 거주지 | 행정 | 재정 |
|---|---|---|---|---|---|---|---|
| 시애틀 (기준) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ WA | ✅ WA |
| 달라스 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ TX | ⏳ |
| SF | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ CA | ✅ CA 세금 |
...
⏳ = 시애틀 fallback 사용 중 → 향후 도시별 차별화 필요

---

## 💡 인사이트 수집·문서화·적용 원칙 (최우선 실행)
...
작업 중 외부 자료(뉴스레터·앱 분석·연구 자료·대화)에서 인사이트 발견 시:

```
1. 즉시 문서화 → docs/ 폴더에 전용 파일 생성
2. CLAUDE.md 변경 이력 섹션에 기록
3. 코드에 바로 적용 (문서만 만들고 끝내지 않음)
4. 적용 결과를 문서에 반영 (현황 업데이트)
```
...
| 파일 | 출처 | 핵심 원칙 | 적용 현황 |
|---|---|---|---|
| `docs/GOAL_SYSTEM.md` | Alignify 교회 목표 가이드 | From X → To Y → By When → Why | ✅ CLAUDE.md + 연간 목표 3개 |
| `docs/DINING_GUIDE.md` | Yelp UX 분석 뉴스레터 | 소비자 관점·카테고리 필터·별점 신뢰 | ✅ 맛집 탭 전면 적용 |
| `docs/LEGAL_ENTITY.md` | LLC vs 비영리 분석 | Washington LLC 먼저, 비영리 나중 | 🔄 설립 대기 중 |
| `docs/OPERATING_PRINCIPLES.md` | Chick-fil-A 모델 | 섬김·탁월함·한 가지 집중 | ✅ 운영 원칙 반영 |
| `docs/CORE_VISION.md` | 폴 김 목사 비전 (2026-05-08) | 문화상품+커뮤니티+교회 이미지 3기둥 | ✅ CLAUDE.md 최상단 고정 |
| `docs/DIASPORA_STRATEGY.md` | BTS 아리랑·김구 「나의 소원」 | 문화의 힘으로 700만 디아스포라 확산 | ✅ 한인 문화 캘린더 + 김구 인용 적용 |
| `docs/LLC_SETUP_WORKFLOW.md` | LLC 설립 실무 | Step by Step 신청·계좌·인보이스 | 🔄 신청 대기 중 |

### Claude Code 행동 원칙

- 사용자가 외부 자료를 공유하면 → **인사이트 추출 후 즉시 문서화**
- 문서화 후 → **코드에 바로 적용** (적용 없는 문서는 의미 없음)
- 적용 후 → **문서에 "✅ 적용 완료" 업데이트**
- 다음 세션에서도 → **이전 인사이트 문서를 참조해 일관성 유지**

---

## 🎯 목표 시스템 (Alignify 기반) — 모든 작업의 나침반

> **상세 문서: `docs/GOAL_SYSTEM.md`**  
...
### 목표 공식 (모든 목표는 이 형식으로만 작성)

```
From X  →  To Y  →  By When  →  And Why
```

### 2026년 연간 목표 3개 (담당: 폴 김 목사)

| # | 목표 | 기한 | Why |
|---|---|---|---|
| 1 | **도시**: From 3 → To 8 active cities | Dec 31 | 재외 한인 700만 섬기기 위한 확장. 뉴욕·내쉬빌·보스턴 목사 관심 표명 — 올해가 적기 |
| 2 | **MAU**: From ~220 → To 1,000 | Dec 31 | 1,000 MAU = 파트너십 신뢰 기준 + 한인 커뮤니티 입소문 임계점 |
| 3 | **수익**: From $200 → To $500/month | Dec 31 | 지속 가능한 운영의 손익분기점. 도메인·서버 비용 자립 |

### Claude Code 작업 시 목표 적용 원칙

기능 추가 전 이 질문을 반드시 확인:
```
① 이 작업이 8개 도시 확장을 지원하는가?
② 이 작업이 MAU 1,000 달성을 지원하는가?
③ 이 작업이 월 $500 수익을 지원하는가?
→ 하나라도 YES면 높은 우선순위 / 모두 NO면 재검토
```
...
| 주기 | 시간 | 내용 |
|---|---|---|
| **매주** | 5분 (기존 미팅 서두) | 3가지 질문: 상태(🟢🟡🔴) / 변경값 / 필요사항 |
| **매월** | 30분 | 4주 패턴 분석 + 계획 조정 |
| **분기** | 2-3시간 | 목표 달성 평가 + 다음 분기 설정 |
| **연간** | 반나절 | 연간 목표 3개 리셋 |

---

## 🔍 Google Search Console (GSC) 운영 원칙

### 핵심 답변: 새 도시 추가 시 GSC 재등록 불필요

```
hebronguide.com 도메인 레벨로 1번 등록 완료
→ /seattle/, /dallas/, /sf/, /boston/ 등 모든 서브패스 자동 포함
→ 새 도시 추가해도 GSC 재등록 필요 없음 ✅
```

### 새 도시 추가 시 해야 할 것 (GSC 관련)

**Step 1: sitemap.xml 업데이트 (필수)**
```
파일: C:\Users\ijigu\...\01_HebronGuide\sitemap.xml
작업: 새 도시 <url> 블록 추가
     - <loc>https://hebronguide.com/{city}/</loc>
     - <lastmod>오늘 날짜</lastmod>
     - 라이브 도시: priority 0.9, changefreq weekly
     - 준비 중 도시: priority 0.8, changefreq monthly
```

**Step 2: GSC에 sitemap 재제출 (선택사항 — 빠른 인덱싱 원할 때)**
```
1. search.google.com/search-console 접속
2. 왼쪽 메뉴 → Sitemaps
3. 기존 sitemap URL 확인 (https://hebronguide.com/sitemap.xml)
4. 이미 등록된 경우 → 재제출 버튼 클릭만으로 충분
5. Google이 자동으로 새 URL 크롤링 (보통 1-4주)
```

**Step 3: URL Inspection (즉시 인덱싱 원할 때)**
```
GSC → URL Inspection → 새 도시 URL 입력 → "Request Indexing"
예: https://hebronguide.com/dallas/
→ 보통 며칠 내 구글 검색에 등장
```

### 현재 GSC 상태 (2026-05-05 기준)

| 항목 | 상태 |
|------|------|
| 도메인 등록 | ✅ hebronguide.com 등록 완료 (목사님 확인) |
| Sitemap 제출 | hebronguide.com/sitemap.xml |
| 라이브 도시 | /seattle/, /dallas/, /sf/ |
| Sitemap 최근 업데이트 | 2026-05-05 (보스턴 추가) |

### 주의: 구 URL 정리

기존에 GitHub Pages로 서빙되던 옛날 URL들이 아직 구글에 인덱스됨.
GSC → Coverage → Excluded → "Crawled, not indexed" 또는 "404" 항목 확인.
필요 시 robots.txt 또는 301 리다이렉트로 정리.
...
> HebronGuide는 이민자·이주자·방문자가 무지(無知)로 손해 보지 않도록 사명적으로 섬기는 플랫폼이다.

### 콘텐츠·기능 추가 시 반드시 따르는 3축 원칙

**① 미국 사회 시스템 활용**  
→ 한인이 모르는 공공자원 발굴 (VITA, 211, Free Clinic, Legal Aid 등)

**② 한인 커뮤니티 연결**  
→ 검증된 한인 업소·기관·교회 큐레이션

**③ 타민족·주류 자원 발굴**  
→ "더 싸고 더 좋은 것"이 있다면 알려줌 (Costco, 99Ranch, Pho, 타코, 인도뷔페 등)

### 모든 콘텐츠 추가 전 확인 질문
```
① 이 정보가 이민자에게 돈을 아끼게 해주는가?
② 이 정보가 이민자에게 시간을 아끼게 해주는가?
③ 이 정보가 이민자의 삶의 질을 높여주는가?
④ 이 정보를 모르면 손해를 보는가?
→ 하나라도 YES면 포함 / 모두 NO면 제외
```

**자세한 내용: `docs/CORE_VALUES.md` 참조**
