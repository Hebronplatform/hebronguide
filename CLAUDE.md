...
| 7 | **도시 수 표기** | 랜딩·정적 HTML → `hebronguide/public/js/hg-config.js`의 `CITY_COUNT` 한 줄 (`.hg-city-count` 자동 갱신) / 앱 마케팅 문자열 "NN개 도시"는 아직 하드코딩(수동) | 숫자 제각각(71·74·69…) |

> 도시 수 SSOT: 랜딩=`hg-config.js` / 앱 홈 통계=`HEBRON_CITIES.length`(자동). 앱 내 "NN개 도시" 문구는 하드코딩이라 도시 추가 시 함께 갱신.
