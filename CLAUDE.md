...
- **이모티콘 금지 (절대 원칙, 2026-07-11 목사님 재확인)**: UI(카드·버튼·섹션 헤더·폼 라벨)에 이모지를 넣지 않는다. 텍스트 라벨·색 배지·CSS 아이콘으로 대체. 기존 데이터에 남은 이모지 접두는 렌더 시 제거(예: ChurchScreen의 `stripEmoji`). 예외: 사용자가 입력한 콘텐츠 원문은 보존.
- **검증 링크 형식**:
```html
