...
cp earth-tex.webp public/earth-tex.webp             # 히어로 지구본 텍스처 (낮면+밤면 한 장)
...
for city in \
  ...
  sed -i.bak \
    ...
    -e "s#시애틀 한인 16.5만이 검증한 정착 가이드. Hebron Platform이 운영하는 신뢰할 수 있는 한인 커뮤니티 플랫폼.#${KO} 한인 커뮤니티를 위한 정착 가이드. 먼저 살아본 사람들이 확인한 신뢰할 수 있는 정보.#g" \
...
# 헤브론 뮤직 신청곡 — ops.html 이 거는 페이지
cp music-requests.html       public/music-requests.html       2>/dev/null || true
